"""Generated relationship model for HAS_IP."""
...
class HasIP(BaseRelationship, IDGenerationMixin):
    """HAS_IP relationship type.
    
    Valid node pairs:    - Domain -> IPAddress    """
    
    # Class attributes
    _rel_type: ClassVar[str] = "HAS_IP"
    schema_version: ClassVar[str] = "0.1"
    _valid_pairs: ClassVar[list[tuple[str, str]]] = [        ("Domain", "IPAddress"),    ]
    ...
    def provide_helpful_errors(cls, values: Any) -> Any:
        ...
        if missing:
            ...
            raise ValueError(
                f"\nHasIP is missing required fields:\n" + 
                "\n".join(missing) +
...
- 'identifying_properties': Dict of identifying properties (or JSON string if serialize_containers=True)
    ...
    def merge(self, other: 'HasIP') -> 'HasIP':
