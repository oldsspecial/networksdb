"""Generated node model for PrivateIPAddress."""
...
from ziptie_schema import trim

from .ip_address import IPAddress


class PrivateIPAddress(IPAddress, IDGenerationMixin):
    """PrivateIPAddress node type.
...
validate_assignment=True
    ...
    # Class attributes
    _primary_label = "PrivateIPAddress"
...
json_schema_extra={"identifying": True, "from_base_schema": False, "property_type": "string", "merge_strategy": "error_if_different", "normalizers": ['ziptie_schema.trim']}
    )
    ...
    def normalize_context(cls, v: Any) -> Any:
        """Apply normalizers to context field."""
        if v is None:
            return v
        
        try:            v = trim(v)
        except Exception as e:
            field_desc = "context string that makes this ip unique"
            raise ValueError(
                f"Failed to process 'context' ({field_desc}): {str(e)}"
            ) from e
        return v
    ...
    def provide_helpful_errors(cls, values: Any) -> Any:
        ...
        if 'context' not in values or values['context'] is None:
            missing.append("  • context: context string that makes this ip unique")

        if missing:
            example_fields = []
            example_fields.append("      address='<value>'")
            example_fields.append("      context='<value>'")

            raise ValueError(
                f"\nPrivateIPAddress is missing required fields:\n" +
                "\n".join(missing) +
                "\n\nExample usage:\n  PrivateIPAddress(\n" +
                ",\n".join(example_fields) +
                "\n  )"
...
- 'identifying_properties': Dict of identifying properties (or JSON string if serialize_containers=True)
    ...
    def merge(self, other: 'PrivateIPAddress') -> 'PrivateIPAddress':
        ...
        # Merge context (strategy: error_if_different)
        self_context = getattr(self, 'context', None)
        other_context = getattr(other, 'context', None)

        if self_context is None and other_context is None:
            merged_data['context'] = None
        elif self_context is None:
            merged_data['context'] = other_context
        elif other_context is None:
            merged_data['context'] = self_context
        else:
            # Both have values, use merge strategy
            strategy = MERGE_STRATEGIES['error_if_different']
            merged_data['context'] = strategy(
                self_context,
                other_context,
                'context'
            )
