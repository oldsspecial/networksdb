"""Generated relationship model for Knows."""
...
class Knows(BaseRelationship, IDGenerationMixin):
    """Knows relationship type.
    
    Valid node pairs:    - EmailAddress -> EmailAddress    """
    
    # Class attributes
    _rel_type: ClassVar[str] = "Knows"
    schema_version: ClassVar[str] = "0.1"
    _valid_pairs: ClassVar[list[tuple[str, str]]] = [        ("EmailAddress", "EmailAddress"),    ]
    ...
    def provide_helpful_errors(cls, values: Any) -> Any:
        ...
        if missing:
            ...
            raise ValueError(
                f"\nKnows is missing required fields:\n" + 
                "\n".join(missing) +
...
- 'identifying_properties': Dict of identifying properties
    ...
    def merge(self, other: 'Knows') -> 'Knows':
